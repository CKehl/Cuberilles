...
uniform sampler2D markTextureXZ;
...
void main()
{
	...
	float markerVal = texture2D(markTextureXZ, gl_TexCoord[1].sp).r;
    ...
    {
		vec4 colour = (1.0-markerVal)*medicalTexture(value) + markerVal*vec4(0,0.68,0,1);
		gl_FragColor = colour;
	}
}
