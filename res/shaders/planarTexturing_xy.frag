...
uniform sampler2D markTextureXY;
...
void main()
{
	...
	float markerVal = texture2D(markTextureXY, gl_TexCoord[1].st).r;
    ...
    {
		vec4 colour = (1.0-markerVal)*medicalTexture(value) + markerVal*vec4(0,0,0.68,1);
		gl_FragColor = colour;
	}
}
